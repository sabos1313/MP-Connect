...
import { adjustStock, deleteIfSafe, listCustomers, listIngredients, listProducts, saveCustomer, saveIngredient, saveProduct, unitOptions } from '../services/dataService';
...
export function EntityPage({ kind }: { kind: EntityKind }) {
  ...
  async function submitStock(event: FormEvent<HTMLFormElement>) {
    ...
    try { await adjustStock({ [kind === 'ingredients' ? 'ingredient_id' : 'product_id']: stockRecord.id, delta: stockType === 'manual_removal' ? -entered : entered, movement_type: stockType, notes: stockNotes }); setNotice('Estoque atualizado e movimentação registrada.'); setModal(null); await load(); }
