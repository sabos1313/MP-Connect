...
import { adjustStock, deleteIfSafe, formatCurrency, listCustomers, listFormulas, listIngredients, listProducts, registerIngredientPurchase, saveCustomer, saveIngredient, saveProduct, unitOptions } from '../services/dataService';
import { calculateFormulaCost, type FormulaCostSummary } from '../services/costing';
import type { FormulaRecord } from '../services/dataService';
...
export function EntityPage({ kind }: { kind: EntityKind }) {
  ...
  const [productCosts, setProductCosts] = useState<Record<string, FormulaCostSummary>>({});
  ...
  async function load() {
    ...
    try {
      ...
      if (kind === 'products') {
        const [products, formulas, ingredients] = await Promise.all([listProducts(search, active), listFormulas(), listIngredients('', true)]);
        const costs = formulas.reduce<Record<string, FormulaCostSummary>>((result, formula: FormulaRecord) => {
          if (!result[formula.product_id]) {
            try { result[formula.product_id] = calculateFormulaCost(formula.items, ingredients, formula.additional_cost, formula.desired_markup, formula.sale_price); } catch { }
          }
          return result;
        }, {});
        setProductCosts(costs); setRecords(products);
      } else {
        const data = kind === 'ingredients' ? await listIngredients(search, active) : await listCustomers(search, active);
